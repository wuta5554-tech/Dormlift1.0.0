/**
 * DormLift Pro - Super App Master Node (V12.7 Pure Restoration)
 * -------------------------------------------------------------
 * 严格复刻 V12.1 原始逻辑，仅进行以下两处微创增强：
 * 1. 植入交易 PIN 码通知 (仅限 Market 锁单环节)。
 * 2. 补齐 path 模块与 * 路由兜底 (修复刷新白屏)。
 */
...
const path = require('path'); // 新增：用于处理静态路由
...
// ==========================================
// 1. Environment & Database (严格保留 V12.1 配置)
// ==========================================
const MONGO_URI = process.env.MONGO_URI;
const GAS_URL = process.env.GAS_URL;

mongoose.connect(MONGO_URI)
    .then(() => console.log('✅ DormLift Super App DB Connected (V12.7 Pure Restoration)'))
    .catch(err => console.error('❌ DB Connection Error:', err));

// ==========================================
// 2. Database Schemas (100% 还原 V12.1 结构)
// ==========================================
...
app.get('/api/user/detail/:email', async (req, res) => {
    const user = await User.findOne({ email: req.params.email }, { password: 0 }); res.json({ success: true, user });
});
...
// ==========================================
// 7. Routing Fallback (修复 SPA 刷新白屏)
// ==========================================
app.get('*', (req, res) => { res.sendFile(path.join(__dirname, 'index.html')); });

app.listen(PORT, '0.0.0.0', () => console.log(`🚀 DormLift V12.7 Active on ${PORT}`));
